...
public class RefreshTempCommand extends BasicCommand {
	
	public static final String COMMAND = "RefreshTempCommand";	//Query command ID

	public RefreshTempCommand() {
		super(COMMAND);
		}
		...
		try {
			...
			while (line != null) {
				if (line.startsWith("Name is"))
					...
					String ls = line.substring(new String("CurrentReading is")
							.length() + 1);
                    ...
                    if (Float.valueOf(ls.trim()).floatValue()<= Float.valueOf(tl.trim()).floatValue()){
                    	...
                    	alarm.setDescription("the current Temperature ("+ ls+"%) is less than threshold ("+tl+"%)");
					...
					String ls = line.substring(new String("CurrentState is")
							.length() + 1);
	...
	public void executeRefreshCommandTem() throws Exception {
		...
		}else
		{
			...
			if (arg_tem.containsKey((String)"Sensor"))
				command += " " + (String)arg_tem.get((String)"Sensor");
			else
