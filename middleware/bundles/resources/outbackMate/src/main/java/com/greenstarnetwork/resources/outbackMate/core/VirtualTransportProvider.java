...
public class VirtualTransportProvider implements IVirtualTransportProvider {
	private String traceAnswer = null;

	public VirtualTransportProvider() {
		traceAnswer = readStringFromFile(this.getClass().getResourceAsStream(
				"/simulator/trace.txt"));
	}
	...
	public byte[] getStreamTransportReponse(byte[] request) {
		...
		else if (telnetMessage.indexOf("Trace") != -1) {
			response = traceAnswer;
		}
		else {
			response = "";
		}
