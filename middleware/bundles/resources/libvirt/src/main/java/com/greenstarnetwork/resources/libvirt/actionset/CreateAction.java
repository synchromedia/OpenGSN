...
/**
 * Create a new VM on a given host
 * @author knguyen
 *
 */
public class CreateAction extends BasicAction{

	public static final String ACTION = "CreateAction";
	...
	private void initializeCommandsList(){
		...
		commandsList.add(CopyTemplateCommand.COMMAND);
		...
		try {
			...
			if (state.getState() == State.INPROGRESS) 
			{
				if (args.containsKey((String)"template")) {
					...
					if (vmTemplate.toLowerCase().compareTo("ofswitch") == 0) {
						commandsList.clear();
						commandsList.add(CreateVMCommand.COMMAND);
						commandsList.add(VMInfoCommand.COMMAND);
						state.setSteps(commandsList);
