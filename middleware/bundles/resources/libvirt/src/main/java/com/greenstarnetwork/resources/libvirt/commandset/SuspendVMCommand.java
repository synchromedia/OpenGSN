 ...
 *
 */
public class SuspendVMCommand extends BasicCommand {
	
	public static final String COMMAND = "SuspendVMCommand";
	...
	public void executeCommand() throws CommandException {
		...
		}else
		{
			...
			if (vmName == null)
				throw new CommandException("Need VM name parameter for PauseVM command!");
			boolean found = false;
			...
			sendCommandToProtocol("virsh -c qemu:///system suspend " + vmName);
		}
	...
	public void parseResponse(IResourceModel model) throws CommandException {
		...
		if (s.indexOf("error:") > -1) {
			throw new CommandException("SuspendVMCommand exception: " + s);
		}else
			((VmHostModel)model).getVM(vmName).setStatus(VMStatus.PAUSED);
	}
