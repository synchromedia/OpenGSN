...
/**
 * On command handler. Turn on outlet(s).
 * @author K.-K.Nguyen <synchromedia.ca>
 *
 */
public class OnCommand extends AbstractCommandWithProtocol {

	public static final String COMMAND = "OnCommand";	//Query command ID
		...
		{		
			...
			if (args.containsKey((String)"Outlet")) {
				...
				for (int i=0; i < outlets.size(); i++) {
					if (outl.equals(outlets.get(i).getID()) || outl.equals(outlets.get(i).getName())) 
					{
						found = true;
						break;
					}
				...
				sendCommandToProtocol(CWGCommandList.ON + " " + outl);
			}else if (args.size() > 0)
				throw new CommandException("Invalid command argument!. Usage:On outlet_ID_or_name");
		}
	}

	@Override
	public void initializeCommand(IResourceModel model) throws CommandException{
		outlets = ((PDUModel)model).getOutlets();
    	initialized = true;
	}
