 ...
 */
package com.greenstarnetwork.services.controller.plan;
...
//		log.debug("nonHostedVMsToMigrateList list: \n" + nonHostedVMsToMigrateList.toXML());
		...
		if ((nonHostedVMsToMigrateList.getVMs()!=null)&&(nonHostedVMsToMigrateList.getVMs().iterator().hasNext())){
			...
			HostList hydroEnergySufficientHostsList = energySufficientHostsListGenerator.getEnergySufficientHostsList(hydroHostList);
			hydroHostInternalInfoList.syncWith(hydroEnergySufficientHostsList);
			
			planGenerator2.execute(hydroEnergySufficientHostsList, hydroHostInternalInfoList,nonHostedVMsToMigrateList, sunWindHostList);
